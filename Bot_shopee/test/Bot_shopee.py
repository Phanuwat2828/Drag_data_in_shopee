...
# from your_module import run, log, get_link, PrintRedirector
...
app.title("Shopee")
app.config(bg="#332941") 
app.geometry("500x700-0+0")  # You can set a default size
...
button_start = tk.Button(button_frame, text="Start Bot", bg="#9ADE7B", fg="white", command=run_system)
button_start.grid(row=0, column=0, padx=5, sticky="ew")

button_stop = tk.Button(button_frame, text="Stop Bot", bg="#7360DF", fg="white", command=stop_program)
button_stop.grid(row=0, column=1, padx=5, sticky="ew")

button_clear = tk.Button(button_frame, text="ลบประวัติทั้งหมด", bg="#FF6868", fg="white", command=log.clearLog)
button_clear.grid(row=0, column=2, padx=5, sticky="ew")

# ****************** Dropdowns and Labels *******************
dropdown_frame = tk.Frame(main_frame)
dropdown_frame.grid(row=1, column=0, columnspan=4, sticky="ew", pady=(0, 10))

dropdown_frame.columnconfigure(0, weight=2)
...
value_num3 = tk.StringVar(value="0")
Working = tk.StringVar()
showStatusBot = tk.StringVar()
showStatusBot.set("สถานะการทำงาน : ยังไม่ทำงาน")

# Options for the first dropdown
options = list(value_link.keys())
selected_value.set(options[0])

# Dropdown 1
dropdown = ttk.Combobox(dropdown_frame, textvariable=selected_value, values=options, state="readonly")
dropdown.grid(row=0, column=0, padx=5, sticky="ew")
dropdown.bind("<<ComboboxSelected>>", on_dropdown_change)

# Dropdown 2
dropdown_num_2 = ttk.Combobox(dropdown_frame, textvariable=selected_value_num_2, state="readonly")
dropdown_num_2.grid(row=0, column=1, padx=5, sticky="ew")
dropdown_num_2.bind("<<ComboboxSelected>>", on_dropdown_change_num2)

# Dropdown 3
dropdown_num_3 = ttk.Combobox(dropdown_frame, textvariable=selected_value_num_3, state="readonly")
...
# Status Label
titleStatusbot = tk.Label(status_frame, textvariable=showStatusBot, anchor="w")
titleStatusbot.pack(fill="x")

# Terminal (ScrolledText)
text = scrolledtext.ScrolledText(main_frame, wrap=tk.WORD, height=10)
text.grid(row=5, column=0, columnspan=4, sticky="nsew")
